...
AboutPars_disp<-  paste0("Dispersal occurs annually when a proportion of sub-adults leave the family territory block (and some remain). The maximum length of the annual dispersal route is 21km. When a dispersing sub-adult cannot gather the minimum area of suitable habitat required for its settlement, the individual is considered stranded and removed from the population.")  
AboutPars_surv<-  paste0("Survival is quantified annually as function of mortality separately for juveniles (0.92), and adults and sub-adults (0.87). Failed dispersal, which occurs when settlement into a new territory was not completed, also results in the death of the sub-adult. The average breeding probability is 0.6; the average litter size is 1.95, with a maximum value of 9 young produced annually per adult pair.")  
AboutTheModel <-  as.character(paste0("The landscape population model of Shirley et al. (2015) predicts the spatial spread of beavers from spatially defined territories and a habitat map. The model is described in detail in the 2015 report but for summary the main processes are outlined here. 
The population model is dynamic and runs on an annual timestep, the spatial dynamics of the beavers integrate with an underlying habitat map to describe territory establishment and sub-adult dispersal.  The habitat map is divided into three categories of habitat: ‘suitable’ habitat which can support a beaver territory; dispersal habitat which cannot support a territory but can be used by beavers; and ‘unsuitable habitat’ which can neither support a territory nor be used by beavers. 

The population dynamics are based around individual beaver families. This is a process-based model, meaning that life-history processes (population dynamics) of mortality, mating, birth, and dispersal are explicitly modelled at the family level on a yearly timestep. Each family consists of a pair of breeding beavers plus zero or more non-breeding adults, subadults, and juveniles. Each family also has a spatially-referenced territory containing sufficient suitable habitat to support the family. Each year, each beaver has a probability of dying equal to the sex-specific mortality rate. Each beaver family with both a breeding male and a breeding female has a probability of producing a litter of kits. 
Some of these kits will join the family as subadults, but others will disperse away from the natal territory. 
