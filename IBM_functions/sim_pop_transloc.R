    ...
    disp <-  departings
       if(length(disp)>0) {
                    ...
                    #   print(disp)
                              ...
                              for (ind.id in 1:length(departings) ) { # mar11 WAS fam.id)) { #fam id same as ind.id bu ind.id seqential, famid will have gaps if some not included

                                 print(paste0("beaver #",ind.id))
                                id <- departings[ind.id]
                                print( paste0("fam # ", id))
                                 depCell <- depCells[ind.id] 
                                 
                                 print(paste0( "depart cell ", depCell))
                                 valid_depCell <- which( values(ter) ==id) # filtered for unsuit hab etc
                                print(  valid_depCell)
                                if(!is.null(cells_make0)){
                                  print(  valid_depCell %in% cells_make0)  
                                 ...
                                 if(length(valid_depCell)==0){
                                               ...
                                               cat("habitat")
                                                         ter <- terra::wrap(ter) 
                                                         disp_compute <- dispersal2(id, fam=families, ter, hab2 , famsize.max, move.max=550, hab.tot.quality=15, routes, mode=mode,disp.fail=0,disp.succ=0,Ndisp=0)  
                                                         
                                                         families <- disp_compute [[1]] 
                                                         ter <- terra::unwrap(disp_compute [[2]] )  
                                                         routes <- rbind(routes, disp_compute [[3]]  )  
                    }#}  
             }
      } 
...
if(sum(values(ter), na.rm=TRUE)==0) {
    print("no group settled  - ")
              simpop_transloc <-  NULL       
              routes0 <- routes   
        
    } else {
    cat("gather init territories  - ")                                   
              routes0 <- routes
              route <- routes <- simpop_transloc <-  NULL 
              ter  <-  subst(ter , 0, NA )      
              habitat_3857  <-  terra::rast(here::here('data',country,'habitat3857.tif'))
...
if(!is.null(routes0)){
  ...
  routes <- NULL
   ...
   hab3857 <-  rast(here::here('data',country,'habitat3857.tif')) 
   pts38577_to4326 <- centreCell3857_to4326(country, xx=multipoints3857[,1], yy=multipoints3857[,2], hab3857)
   cat("coords conversion ok  - ")
   routes <-  data.frame(lng=pts38577_to4326$X,  lat=pts38577_to4326$Y,  id=routes$id,  path=routes$path, stage=routes$stage)
}
