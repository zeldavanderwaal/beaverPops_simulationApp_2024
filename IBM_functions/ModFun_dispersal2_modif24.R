...
notPreviousfunc <- function(id, fam, ter, hab, famsize.max, move.max, hab.tot.quality, routes, mode,disp.fail,disp.succ,Ndisp) {
  ...
  Ndisp <- Ndisp
   ...
   ter2 <- terra::as.matrix(ter, wide = T) # wide == each row in the SpatRaster becomes a row in the matrix and each column in the SpatRaster becomes a column in the matrix
...
#  print(which( values(ter) ==id))
 ...
 #  if( mode %in% c("settlement_test_but_not", "settlement_test")) { 
      ...
      print(paste0("located fam #",id," translocated, departing from cell #",depCell))
...
#       print( paste0("fam #",id,"///bvr #",bvr ))        
 ...
 #       print(which( values(ter) ==id))
  # }
   path <- NULL
#   habwrapped <- terra::wrap(hab)
 #   if( mode == "settlement_test") { # not wandering for sett test but for translocated in model and normal, YES!
 #      path <- NULL
 #      } else {
 #      if( mode == "settlement_test_but_not") { # not wandering for sett test but for translocated in model and normal, YES!
 #      path <- NULL
 #       path  <- wander2(id, hab=hab2, origin= depCell) 
   #    if( mode %in% c("settlement_test_but_not", "settlement_test")) {
        path  <- wander2(id, hab=hab2, origin= depCell)
    #  } else { 
          cat( " - wander from? - ")
...
##      print(path)
##      print("test")
    #           path <- wander2(id, hab=hab2,  origin= which(ter2==id))# start.here)  ### Added Mar 2020
##      print(path)
#       print("old")
#       print(which(ter2==id))
#       print(which(values(ter)==id))
...
#       
  ...
  ## add lock here not async
   startf.natal <- startf <- adjacent.id(ter, cells=depCell  , id=id, directions=8)
   start.here <- psample(find.best.hab(startf.natal, hab2,depCell  ), 1) # KNOWN ORIGIN
  if(sum(values(hab)[start.here] )==0) {start.here <- NULL}
...
#may8 readd                      data.frame(bvr=bvr, cell=path,  move=0, stage="wander")) 
  ...
  ######## if no cells around
        if(length(start.here)==0  ) {
            ...
            ter <- terra::wrap(ter)  
            routes  <- rbind(routes.loc, data.frame(bvr=bvr, cell=0, move= move.max, stage="fail"))
        #          routes.loc <-rbind( routes.loc  ,data.frame(bvr=id, cell=depCell,  move=0, stage="depart"))
  ...
  start.here <- sample(path,1) #path[length(path)]
...
#  startf.natal <- unique(c(startf))
  ...
  startf <- adjacent.id(ter, cells=start.here, id=id, directions=8)
#  startf <- adjacent.id(ter, cells=which(ter2==id), id=id, directions=8)
  startf.natal <- unique( c(startf)) # wasc()
...
# unsuitable
    if (is.na(hab.here) | hab.here==0) {
      ...
      depCell <- which(ter2==id) # which( values(ter) ==id) # which            
...
# suitable, occupied  
      ...
      #try to join this territory
       if (verbose) cat("\tbeaver from", id, ": discovered territory", ter.here, "\n")  
            ...
            print(paste0("join? famsize is:", famsize))
...
#                       
            }  else if (hab.here==2 & ter.here > 0  & mode =="settlement_test"){ 
