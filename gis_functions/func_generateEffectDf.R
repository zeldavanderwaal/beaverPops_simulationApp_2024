 cat("generate effectDf  - ")
 
#### generate effect table for sim ####
## a table with effect (LeafletId) in first col, and time (min-max) in other cols
...
generateEffectDf <- function(metaDat, layerEffect) {
  ...
  cat( paste0(NFeat_metadat,  "feat in data  -")  ) 
        if(NFeat_metadat>0){ 
